...
export default function SearchBar({ onFilter }: SearchBarProps) {
  ...
  const handleSearchClick = () => {
    ...
    // Call onFilter prop if provided
    if (onFilter && selectedServices.length > 0) {
      onFilter({ category: selectedServices.join(',') });
    }
            ...
            {SERVICE_OPTIONS.map(option => {
              ...
              return (
                <label
                  ...
                  onMouseOver={e =>
                    !isChecked && (e.currentTarget.style.backgroundColor = '#f5f5f5')
                  }
                  onMouseOut={e =>
                    !isChecked && (e.currentTarget.style.backgroundColor = 'transparent')
                  }
                >
                  <input
                    type="checkbox"
